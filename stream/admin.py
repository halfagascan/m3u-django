...
class ProgramGuideAdmin(admin.ModelAdmin):
    ...
    ordering = ['start_time']
from .models import Channel, Programmes, EPGChannels
...
def ProtectChannel(modeladmin, request, queryset):
    queryset.update(channel_protected=True)
ProtectChannel.short_description = "Protect Channel from auto-delete"

class ChannelAdmin(admin.ModelAdmin):

    def ProgrammeCount(self, obj):
        return Programmes.objects.filter(Channel=obj.tvg_id).count()
    ProgrammeCount.short_description = "Imported Programmes"
    list_display = ('epg_id', 'channel_name','channel_group','channel_enabled','ProgrammeCount','tvg_id')
    ordering = ('epg_id', 'id')
    actions = [ProtectChannel,enable_channel, disable_channel,change_cat_ENTERTAINMENT,change_cat_COMEDY,change_cat_DRAMA,change_cat_MOVIES,change_cat_MUSIC,change_cat_NEWS,change_cat_SPORTS,change_cat_TRAVEL]
...
class EPGAdmin(admin.ModelAdmin):
    list_display = ('ChannelName', )

admin.site.register(EPGChannels, EPGAdmin)
